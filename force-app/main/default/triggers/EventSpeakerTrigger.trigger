trigger EventSpeakerTrigger on EventSpeakers__c (before insert, before update ) {
    ...
    for( EventSpeakers__c es : Trigger.New ){ //whenever the new speaker and event are created they added to Eventspeaker 
    ...
    //Retriving the records for the date and time for the event
    ...
    for( EventSpeakers__c es : Trigger.New ){ // - Salesforce Geek
        ...
        for(EventSpeakers__c es1 : relatedEventSpeakerList) {
            ...
            //Checking whether the speaker and startDateTime are same or not
            if(es1.Speaker__c == es.Speaker__c && es1.Event__r.Start_DateTime__c == bookingTime ){
                
                //If the conditio is satisfied then the error is displayed 
